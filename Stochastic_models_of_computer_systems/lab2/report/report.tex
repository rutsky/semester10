...
H_0 \  \mathrm{\text{принимается}} \iff
    \mathcal{N}_{\frac{\alpha}{2}} < 
	    ...
	    \mathcal{N}_{1 - \frac{\alpha}{2}}.
$$
% TODO: Как заметил Толмачев Д., использование и верхнего, и нижнего \alpha/2 
% квантилей для критерия совсем не обязательно - достаточно только верхнего 
% \alpha квантиля, т.к. заявки могут попадать только в верхний квантиль.
...
\tau_2 = 1 - \tau_1.$
% TODO: Здесь производится нетривиальная аппроксимация:
% Количество заявок, которое приходит в момент t_i распределено по Пуассону,
% мы же сужаем множество возможных событий до двух: прибытие или отсутствие
% одной заявки и аппроксимируем распределение Пуассона Биномиальным 
% распределением.
...
вектор $\theta$ примем за результат оценки.
