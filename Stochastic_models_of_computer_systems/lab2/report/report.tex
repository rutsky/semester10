...
$$
где $\mathbb{I}(expr)$~--- функция индикатор,%
\footnote{%
...
Пусть имеется начальная оценка параметров $\theta$: $\theta_0$ 
(использованный способ вычисления $\theta_0$ будет описан ниже).
Последовательно выполняя E- и M-шаги будем уточнять оценку $\theta_k$,
пока она не сойдётся к какой-то величине 
$\theta_k \xrightarrow[k \rightarrow \infty]{} \theta$, 
